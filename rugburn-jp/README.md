...
Rugburn is a replacement for `ijl15.dll` that allows you to run unmodified PangYa™
(URL/port rewrites, updater bypass). The `enable_gameguard` branch leaves GameGuard
enabled: it does **not** block `GameGuard.des` and does **not** patch the client
GG/HS check routines (including JP 983).
...
Features:
  ...
  * `master` disarms GameGuard / HackShield. This branch (`enable_gameguard`)
    does not: GameMon is allowed to start and the JP/US/… check patches are skipped.
